...
{
    vector<PPoint3D> points = t.getPoints();
    this->points.push_back( new Point3D( *(points[0]) ) );
    this->points.push_back( new Point3D( *(points[1]) ) );
    this->points.push_back( new Point3D( *(points[2]) ) );
}
...
vector<PPoint3D> Triangle::getPoints() const
{
    return this->points;
}
